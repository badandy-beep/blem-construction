...
export default function ClientAccessPage() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (password === 'blem2026') { setIsAuthenticated(true); setError(''); } 
      ...
      <div style={{ fontFamily: "'Outfit', sans-serif", backgroundColor: '#1C2B39', color: '#FFFFFF', minHeight: '100vh', paddingTop: rs('120px', '100px') }}>
        <section style={{ padding: rs('80px 5%', '50px 16px') }}>
          <div style={{ maxWidth: '800px', margin: '0 auto', textAlign: 'center' }}>
            <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '32px' }}><LogoMonogram size={rs(80, 60)} variant="primary" /></div>
            ...
            <p style={{ fontSize: rs('18px', '15px'), color: 'rgba(255,255,255,0.7)', marginBottom: '48px' }}>Direct, secure access—streamlining communication.</p>
            <div style={{ display: 'grid', gridTemplateColumns: rs('repeat(3, 1fr)', '1fr'), gap: rs('24px', '16px'), marginBottom: '48px' }}>
              {[{ icon: '📐', title: 'Blueprints', desc: 'Access project drawings' }, { icon: '📝', title: 'Documents', desc: 'Contracts and specs' }, { icon: '📊', title: 'Progress', desc: 'Track milestones' }].map((item, i) => (
    ...
    <div style={{ fontFamily: "'Outfit', sans-serif", backgroundColor: '#1C2B39', color: '#FFFFFF', minHeight: '100vh', paddingTop: rs('120px', '100px'), display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ maxWidth: '450px', width: '100%', padding: '0 16px' }}>
        <div style={{ textAlign: 'center', marginBottom: '40px' }}>
          <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '24px' }}><LogoMonogram size={rs(70, 60)} variant="primary" /></div>
        ...
        <form onSubmit={handleSubmit} style={{ backgroundColor: 'rgba(255,255,255,0.05)', padding: rs('40px', '24px'), borderRadius: '16px', border: '1px solid rgba(255,255,255,0.1)' }}>
          {error && <div style={{ backgroundColor: 'rgba(220,38,38,0.1)', border: '1px solid rgba(220,38,38,0.3)', padding: '12px 16px', borderRadius: '8px', marginBottom: '24px', fontSize: '14px', color: '#FCA5A5' }}>{error}</div>}
          <div style={{ marginBottom: '24px' }}>
            ...
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter password" style={{ width: '100%', padding: '18px', border: '1px solid rgba(255,255,255,0.2)', fontSize: '16px', fontFamily: "'Outfit', sans-serif", backgroundColor: 'rgba(255,255,255,0.05)', color: '#FFFFFF', borderRadius: '8px' }} />
          </div>
        ...
        <p style={{ textAlign: 'center', marginTop: '24px', fontSize: '14px', color: 'rgba(255,255,255,0.5)' }}>Don't have a password? <a href="tel:9417772536" style={{ color: '#B8860B' }}>(941) 777-BLEM</a></p>
      </div>
    </div>
  );
}
